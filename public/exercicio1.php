<?php
// Aula de Lógica de Programação - Professor Marcos Monteiro marcos.monteiro@gmail.com
echo "<h1>Exercício de Algoritmo 1 </h1>";
echo "<h2>Como trocar um pneu furado ?</h2>";
echo "<ol>";
echo "<li>Se estiver dentro do carro, Sair do carro;</li>";
echo "<li>Sinalizar na Rua;</li>";
echo "<li>Afrouxar levemente os parafusos do pneu furado;</li>";
echo "<li>Subir o carro com o Macaco;</li>";
echo "<li>Retirar o pneu furado do carro;</li>";
echo "<li>Retirar o pneu estepe do local de armazenamento;</li>";
echo "<li>Colocar o pneu estepe no lugar do pneu furado;</li>";
echo "<li>Guardar o pneu furado para reparo futuro.</li>";
echo "</ol>";
echo "<button onclick=\"window.location.href = './index.php';\">Voltar p/ home</button>";
